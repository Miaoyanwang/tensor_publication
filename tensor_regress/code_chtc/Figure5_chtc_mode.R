...
####### Binary #######
args <- commandArgs(T) # signal, rank, info, dup
...
d=20; whole_shape = rep(d,3); dup = 30
## combinations of signal and rank
signal_range=c(3,6)
core_range=rbind(c(3,3,3),c(4,5,6))

err_res = err_env = err_rrr = err_mreg1 = c()
cor_res = cor_env = cor_rrr = cor_mreg1 = c()
...
save(result, file = paste0("s_",s,"_r_",r,"_i_",i,"_n_",n,"_fig5_mode.RData") )


# save data to matlab
# need to transpose the data
tsr = tensor_trans(dat)
x = data$X_covar3
u = tensor_trans(Ey)
para = c("signal", signal_range[s], "rank",core_shape,"info",i,"dup",n)

sample_list = list(tsr = tsr, y = x, x_true = u, para = para)
write.mat(sample_list,paste0("s_",s,"_r_",r,"_i_",i,"_n_",n,"_fig5_mode.mat"))