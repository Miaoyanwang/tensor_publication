...
d=20;
whole_shape = rep(d,3);
final_mode=array(0,dim=c(2,2,3,4))
final_sd_mode=array(0,dim=c(2,2,3,4))
...
for(s in 1:2){ ## signal level
    for(r in 1:2){ ## rank
        for(i in 1:3){ ## number of informative covariates
            ...
            final_mode[s,r,i,2]=mean(err_holrr[s,r,i,])
            final_mode[s,r,i,3]=mean(err_pls[s,r,i,])
            final_mode[s,r,i,4]=mean(err_tpg[s,r,i,])
            
            
            final_sd_mode[s,r,i,1]=sd(err_res[s,r,i,])
            final_sd_mode[s,r,i,2]=sd(err_holrr[s,r,i,])
            final_sd_mode[s,r,i,3]=sd(err_pls[s,r,i,])
            final_sd_mode[s,r,i,4]=sd(err_tpg[s,r,i,])
        }
    }
}
...
#### run simulation: PMSE vs. dimension
set.seed(0)
seed=0
## combinations of signal and rank
signal_range=c(3,6)
...
##
dup=30;
final_sample=array(0,dim=c(2,2,5,4))
final_sd_sample=array(0,dim=c(2,2,5,4))
...
for(s in 1:2){ ## signal level
    for(r in 1:2){ ## rank
        for(i in 1:5){ ## dimension with informative mode
            ...
            final_sample[s,r,i,2]=mean(err_holrr[s,r,i,])
            final_sample[s,r,i,3]=mean(err_pls[s,r,i,])
            final_sample[s,r,i,4]=mean(err_tpg[s,r,i,])
            
            
            final_sd_sample[s,r,i,1]=sd(err_res[s,r,i,])
            final_sd_sample[s,r,i,2]=sd(err_holrr[s,r,i,])
            final_sd_sample[s,r,i,3]=sd(err_pls[s,r,i,])
            final_sd_sample[s,r,i,4]=sd(err_tpg[s,r,i,])
        }
    }
}

save(final_mode,final_sd_mode,final_sample,final_sd_sample,file="Figure5.RData")
