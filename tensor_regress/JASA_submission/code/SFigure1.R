## Code for Supplementary Figure 1. Comparison of mean sqaured prediction error between our method and the other three previous methods
...
pdf("SFigure1.pdf",width=20.5,height=3)
s=r=1;
...
s=2;r=2;
data=data.frame(PMSE=c(final[s,r,1,],final[s,r,2,],final[s,r,3,])/signal_range[s],sd=c(finalsd[s,r,1,],finalsd[s,r,2,],finalsd[s,r,3,])/signal_range[s],Method=rep(c('STD (Our method)','HOLRR (Low-rank regression)','HOPLS (Partial least sqaure)','TPG (Projection gradient)'),3),Category=c(rep(5,4),rep(10,4),rep(15,4)))
data[,3]=factor(data[,3],levels=c("STD (Our method)","HOLRR (Low-rank regression)","HOPLS (Partial least square)","TPG (Projection gradient)"))
...
save(final,finalsd,file="SFigure1.RData")
