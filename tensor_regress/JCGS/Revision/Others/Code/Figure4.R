...
data=data.frame(PMSE=c(final[s,r,1,],final[s,r,2,],final[s,r,3,]),sd=c(finalsd[s,r,1,],finalsd[s,r,2,],finalsd[s,r,3,])/sqrt(30),Method=rep(c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"),3),Category=c(rep(1,5),rep(2,5),rep(3,5)))
data[,3]=factor(data[,3],levels=c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"))
p2=ggplot(data=data, aes(x=as.factor(Category),y=PMSE, fill=Method))+geom_bar(stat="identity", position=position_dodge())+geom_errorbar(aes(ymin=PMSE-sd, ymax=PMSE+sd), width=.2,position=position_dodge(.9))+labs(x="Number of modes with available features",y="Misalignment error")+coord_cartesian(ylim = c(0,0.5)) +  labs(title = "Low Signal, High Rank",size = 5) +theme(plot.title = element_text(hjust = 0.5,size = 11))+
...
data=data.frame(PMSE=c(final[s,r,1,],final[s,r,2,],final[s,r,3,]),sd=c(finalsd[s,r,1,],finalsd[s,r,2,],finalsd[s,r,3,])/sqrt(30),Method=rep(c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"),3),Category=c(rep(1,5),rep(2,5),rep(3,5)))
data[,3]=factor(data[,3],levels=c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"))
p3=ggplot(data=data, aes(x=as.factor(Category),y=PMSE, fill=Method))+geom_bar(stat="identity", position=position_dodge())+geom_errorbar(aes(ymin=PMSE-sd, ymax=PMSE+sd), width=.2,position=position_dodge(.9))+labs(x="Number of modes with available features",y="Misalignment error")+coord_cartesian(ylim = c(0, 0.5)) +  labs(title = "High Signal, Low Rank",size = 5) +theme(plot.title = element_text(hjust = 0.5,size = 11))+
  scale_fill_manual(values=new_color)+theme(axis.text=element_text(size=12),axis.title=element_text(size=10))
#p3
...
data=data.frame(PMSE=c(final[s,r,1,],final[s,r,2,],final[s,r,3,]),sd=c(finalsd[s,r,1,],finalsd[s,r,2,],finalsd[s,r,3,])/sqrt(30),Method=rep(c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"),3),Category=c(rep(1,5),rep(2,5),rep(3,5)))
data[,3]=factor(data[,3],levels=c('STD (Our method)',"SupCP","Envelope","mRRR","GLSNet"))
p4=ggplot(data=data, aes(x=as.factor(Category),y=PMSE, fill=Method))+geom_bar(stat="identity", position=position_dodge())+geom_errorbar(aes(ymin=PMSE-sd, ymax=PMSE+sd), width=.2,position=position_dodge(.9))+labs(x="Number of modes with available features",y="Misalignment error")+coord_cartesian(ylim = c(0, 0.5)) +  labs(title = "High Signal, High Rank",size = 5) +theme(plot.title = element_text(hjust = 0.5,size = 11))+
  scale_fill_manual(values=new_color)+theme(axis.text=element_text(size=12),axis.title=element_text(size=10))
#p4


### dim vs 1 - cor
load("presaved/fig4_sample.RData")
...
p6=ggplot(data, aes(x = d*400, y = PMSE)) + geom_line(aes(color = Method),size = 1.2)  + scale_colour_manual(values = new_color)+
  geom_point(size = 2,aes(shape = Method))+ scale_shape_manual(values = c(16,5,17,4,1,2)) + theme(axis.text=element_text(size=12), axis.title=element_text(size=10))+ xlab('Sample Size') + labs(title="Low Signal, High Rank") + ylab("Misallignment error")+ theme(plot.title = element_text(hjust = 0.5,size = 11))+coord_cartesian(ylim = c(0,0.6))
...
p7=ggplot(data, aes(x = d*400, y = PMSE)) + geom_line(aes(color = Method),size = 1.2)  + scale_colour_manual(values = new_color)+
  geom_point(size = 2,aes(shape = Method))+ scale_shape_manual(values = c(16,5,17,4,1,2)) + theme(axis.text=element_text(size=12), axis.title=element_text(size=10))+ xlab('Sample Size') + labs(title="High Signal, Low Rank") + ylab("Misallignment error")+ theme(plot.title = element_text(hjust = 0.5,size = 11))+coord_cartesian(ylim = c(0,0.6))
...
p8=ggplot(data, aes(x = d*400, y = PMSE)) + geom_line(aes(color = Method),size = 1.2)  + scale_colour_manual(values = new_color)+
  geom_point(size = 2,aes(shape = Method))+ scale_shape_manual(values = c(16,5,17,4,1,2)) + theme(axis.text=element_text(size=12), axis.title=element_text(size=10))+ xlab('Sample Size') + labs(title="High Signal, High Rank") + ylab("Misallignment error")+ theme(plot.title = element_text(hjust = 0.5,size = 11))+coord_cartesian(ylim = c(0,0.6))
p8=p8+geom_errorbar(aes(ymin=PMSE-sd,ymax=PMSE+sd),width=0.5,position=position_dodge(0.05))
#p8

pdf("Figures/Figure4.pdf", width = 9, height = 6)
(p2|p3)/
  (p6|p7)
dev.off()
  ...
  for (r in 1:2) { # rank
    ...
    for (i in 1:3) { # # of informative modes
      ...
      for (n in 1:dup) { 
        ...
        err_env[s,r,i,n]= mean((env_fit$fitted.values@data - data$U)^2)
        cor_env[s,r,i,n]= cor(as.vector(env_fit$fitted.values@data), as.vector(data$U))
        ...
        err_rrr[s,r,i,n]= mean((rrr_fit$mu - U0)^2)
        cor_rrr[s,r,i,n]= cor(as.vector(rrr_fit$mu), as.vector(U0))
        
        # save data to matlab
        # need to transpose the data
        tsr_list[[n]] = tensor_trans(data$tsr[[n]])
        x_list[[n]] = data$X_covar3
        u_list[[n]] = tensor_trans(data$U)
...
write.mat(sample_list,"mat_data/mode_fig4_v1.mat")

#load("for_chtc/chtc_results/mode_fig4_v1.RData")
# read matlab results
supcp_result = read.mat("mat_output/fig4_mode.mat")
...
save(final_mode,final_sd_mode,final_mode_cor,final_sd_mode_cor,file = "presaved/fig4_mode.RData")
  ...
  for (r in 1:2) { # rank
    ...
    for (i in 1:5) { # dimension with informative sample
      ...
      for (n in 1:dup) { 
        ...
        x_list[[n]] = data$X_covar3
        u_list[[n]] = tensor_trans(data$U)
        para_list[[n]] = c("signal", signal_range[s], "rank",core_shape,"dim",d[i],"dup",n)
      }# end dup
...
sample_list = list(tsr = tsr_list_s, y = x_list_s, x_true = u_list_s, para = para_list_s)
write.mat(sample_list,"mat_data/fig4_sample_v1.mat")

#load("for_chtc/chtc_results/sample_fig4_v1.RData")

# read matlab result
supcp_result = read.mat("mat_output/fig4_sample.mat")
...
save(final_sample,final_sd_sample,final_sample_cor,final_sd_sample_cor,file = "presaved/fig4_sample.RData")
