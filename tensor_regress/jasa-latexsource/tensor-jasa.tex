...
\bigskip
\begin{abstract}
We consider the problem of decomposing a high-order tensor with exponential-family entries, given side information on multiple modes. Such data problems arise frequently in applications such as neuroimaging, network analysis, and spatial-temporal modeling. We propose a new family of tensor response regression that treats side-information as incorporate covariates and we establish the accuracy guarantees for our proposed estimation. Unlike earlier methods on unsupervised tensor decomposition, our supervised approach allows high-dimensionality in both the tensor data and the covariate matrices on multiple modes. An efficient alternating updating algorithm is further developed. Our proposal handles a broad range of data types, including continuous, count, and binary observations. Through simulation and applications to two real datasets, we demonstrate the outperformance of our approach over the state-of-art.
\end{abstract}
