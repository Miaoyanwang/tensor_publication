...
for(s in 1:2){
    for(r in 1:2){
        ...
        p=ggplot(data=data, aes(x=as.factor(Category),y=PMSE, fill=Method)) +
        geom_bar(stat="identity", position=position_dodge())+
        geom_errorbar(aes(ymin=PMSE-sd, ymax=PMSE+sd), width=.2,
        position=position_dodge(.9))+labs(x="Number of modes with available features",y="MSPE")+coord_cartesian(ylim = c(0, max(final[s,r,,])+0.05)) +  labs(title = title,size = 5) +
        theme(plot.title = element_text(hjust = 0.5,size = 15))+scale_fill_manual(values=c(rev(wes_palette(n=4, name="Moonrise2"))[1:3],'#C0C0C0'))+theme(axis.text=element_text(size=12),axis.title=element_text(size=12))
        plot(p)
