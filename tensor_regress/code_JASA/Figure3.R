...
pdf("F1_normal.pdf",width=5,height=4)
...
pdf("F1_binary.pdf",width=5,height=4)
figure=ggplot(res, aes(x =d^3/d, y = MSE)) + geom_line(aes(color = as.factor(r)),size = 1)+geom_point(size=1)+theme(axis.text=element_text(size=12),axis.title=element_text(size=14))+labs(x="effective sample size",y="mean sqaured error (MSE)")
...
load("presaved/F1_poisson.RData")
table=F1_poisson
...
save(F1_normal,file="presaved/F1_normal.RData")
...
save(F1_binary,file="presaved/F1_binary.RData")
...
save(F1_poisson,file="presaved/F1_poisson.RData")
