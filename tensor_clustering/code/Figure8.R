# Code for Figure 8. Comparison among HLloyd, HOSVD, HOSVD+, SCORE
# with varying signal exponent under different shape parameters in heterogeneity
...
# if a new run of simulation is desired, please go to line #90 to run the code --------------------------------------

# a6

load("presaved/Figure8_a6.RData")
...
p7 <- ggplot(data = data, aes(x = gamma, y = MCR)) +
...
p7 <- p7 + geom_errorbar(aes(ymin = MCR - sd, ymax = MCR + sd), width = 0.02, position = position_dodge(0.05)) + theme_light() +
...
load("presaved/Figure8_a2.RData")
...
p8 <- ggplot(data = data, aes(x = gamma, y = MCR)) +
  geom_line(aes(color = Algorithm), size = 1.2) +
  scale_colour_manual(values = sel_color) +
  geom_point(size = 3, aes(shape = Algorithm)) +
  scale_shape_manual(values = c(16, 17, 5, 2, 4)) +
  ...
  guides(shape = guide_legend(order = 1), color = guide_legend(order = 1))
p8 <- p8 + geom_errorbar(aes(ymin = MCR - sd, ymax = MCR + sd), width = 0.02, position = position_dodge(0.05)) + theme_light() +
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 16), plot.title = element_text(hjust = 0.5, size = 16), legend.title = element_text(size = 14), legend.text = element_text(size = 14))


pdf("figure/Figure8.pdf", height = 4, width = 10)
(p7 | p8)
dev.off()
...
for (g in 1:length(gamma_range)) { # gamma
  ...
  for (n in 1:dup) { # dup
    ...
    dat <- sim_dTBM(
      ...
      dist = dist, sigma = 0.75, theta_dist = theta_dist, alpha = 6, beta = (6 - 1) / 6
    )
...
save(mis, mis_sd, file = "presaved/Figure8_a6.RData")
...
for (g in 1:length(gamma_range)) { # gamma
  ...
  for (n in 1:dup) { # dup
    ...
    dat <- sim_dTBM(
      seed = seed, imat = F, asymm = F, p = rep(p, 3), r = rep(r, 3),
      core_control = "control", delta = delta, s_min = s_min, s_max = s_max,
      dist = dist, sigma = 0.75, theta_dist = theta_dist, alpha = 2, beta = (2 - 1) / 2
    )
...
save(mis, mis_sd, file = "presaved/Figure8_a2.RData")
