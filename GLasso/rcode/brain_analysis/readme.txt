...
3. generate_funcs.R : R code to generate the sample covariance matrices and their figures with given gene expression data
...
5. pipeline.m : Matlab code to obtain the algorithm results using Joint_tucker_v6.m; 
...
Output: U.csv(membership matrix), Theta0.csv(global connection), Theta_l.csv(l = 1,...,r, tissue specified connection), Obj_curve.csv(trajectory of objective value)
