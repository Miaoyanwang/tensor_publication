function [Omega,U,Theta0,Theta,convg,rec_obj]=Joint_tucker_v6(SS,nn,r,option)
...
%
%   U       K*r matrix, with unit-norm, strictly orthogonal (nonoverlapping) sparse columns, each column gives a
%           cluster of the K populations
%
...
while niter<Niter  && abs(diff)>Tol
    ...
    % estimate U (with L0) and Theta (with L1) , together (not sequentially)
    ...
    for l=1:r
        ...
        for ind_u=1:length(keep) % keep #
            for ind_v=1:seg % larger=sparser
                % est u with L0
                curr_u=normalize(thres(rawu,ind_u),'norm'); % L0 hard thresholding
...
%                 BIC(ind_u,ind_v)=log(norm(curr_mat-curr_u*curr_Theta(:)','fro')^2/numel(curr_mat))+log(numel(curr_mat))*(ind_u*sum(curr_Theta(:)~=0))/numel(curr_mat);
        ...
        % best u and theta
        curr_u=normalize(thres(rawu,opt_u),'norm');
