  ...
  for(nsim in 1:rep){
    ...
    while((iter < 20)&(error >10^-3)){
      ...
      #### block model
           beta1=beta2=0
      for(i in 1:n){
         beta1=beta1+alpha[i]*y[i]*P_row%*%t(P_row)%*%X[[i]]
         beta2=beta2+alpha[i]*y[i]*X[[i]]%*%P_col%*%t(P_col)
      }
      beta=beta1+beta2
      P_row=model.matrix(~-1+as.factor(kmeans(beta,r,nstart=5)$cluster))
      P_col=model.matrix(~-1+as.factor(kmeans(t(beta),r,nstart=5)$cluster))
      P_row=svd(P_row)$u ## normalize
      P_col=svd(P_col)$u ## normalize
### newly added.
...
## newly added
indset = matrix(0,nrow =68 ,ncol = 2)
s = 0
for(i in 1:68){
        s = s+1
        indset[s,]= c(i,1)
...
for(k in 1:5){
  ...
  cvresult[k,2] = sum(hloss[train_index]);
  cvresult[k,4] = sum(hloss[test_index])
...
save(cvresult,file = paste("CV",r,"_",cost,".RData",sep = ""))

h=NULL
cost=1
for(r in 1:68){
    load(paste("CV",r,"_",cost,".RData",sep = ""))
    h=c(h,mean(cvresult[,3]))
}
plot(h,type="l")
