...
\begin{thm}[Markov's inequality]\label{thm:markov}
	...
	\begin{equation}\label{eq:thmmarkov}
		\bbP(X\geq t) \leq \frac{E[X]}{t},\quad \text{ for all } \ t > 0.
	\end{equation}  
\end{thm}
...
\begin{thm}[Markov's inequality for polynomial moments]\label{thm:polymoment}
	...
	\begin{equation}\label{eq:poly}
		\bbP(|X - \mu |\geq t) \leq \frac{\bbE[|X-\mu|^k]}{t^k},\quad \text{ for all } \ t > 0.
	\end{equation}  
\end{thm}
...
\begin{thm}[Chernoff bound]\label{thm:chernoff}
	...
	\begin{equation}\label{eq:chernoff1}
		\bbP((X - \mu) \geq t) \leq \frac{\bbE[e^{\lambda (X-\mu)}]}{e^{\lambda t}}, \quad \text{ for all } \lambda\in (-b,b).
	\end{equation}
...
Let $X \sim sG(\sigma^2)$. For all integer $k \geq 1$, we have
...
\begin{thm}[One-sided Bernstein’s inequality]\label{thm:oneber}
 Let $X$ be a random variable. If $X \leq b$ almost surely, then 
\begin{equation}\label{eq:thmoneber1}
	\bbE[e^{\lambda (X-\bbE[X])}] \leq exp\left\{ \frac{\lambda^2 \bbE[X^2]/2}{1 - b\lambda /3}  \right\}, \quad \text{for all } \lambda \in [0, 3/b).
\end{equation}
...
\begin{equation}\label{eq:thmoneber2}
	\bbP[\sum_{i = 1}^n (X_i - \bbE[X_i] ) \geq n\delta ] \leq exp\left\{ - \frac{n\delta^2}{\sum_{i = 1}^n \bbE[X_i^2] /n + b \delta /3} \right\},\quad \text{ for all } \delta \geq 0.
\end{equation}  
Particularly, suppose there are $n$ independent non-negative variables $Y_i \geq 0$, for all $i \in [n]$. The equation~\eqref{eq:thmoneber2} becomes
\begin{equation}\label{eq:thmonber0}
	\bbP[\sum_{i = 1}^n (Y_i - \bbE[Y_i] ) \leq n\delta ] \leq exp\left\{ - \frac{n\delta^2}{\sum_{i = 1}^n \bbE[Y_i^2] /n} \right\},\quad \text{ for all } \delta \geq 0.
\end{equation} 
\end{thm}
...
\item[(a)] For a given constant $t >0$, we define a variable $Y_{t} = X - t\mI_{[X\geq t]}$, where $\mI$ is the indicator function. Noted that $Y_{t}$ is a nonnegative variable, the Markov's inequality follows by taking expectation to $Y$,
\begin{equation}
...
\begin{rmk}[Tightness of Markov's inequality]
	Only a few variables can attain the equalities in Markov's and Chebyshev's inequalities. In research, we should pay attention to the concentration bounds tighter than Markov's inequality.
\end{rmk}
...
\[\quad \phi'(z) = -z \frac{1}{\sqrt{2\pi}} \exp(-\frac{z^2}{2} ) = -z \phi(z). \] 
	...
	Combing equations~\eqref{eq:e22pro1}, \eqref{eq:e22pro2}, and  \eqref{eq:e22pro3}, we have $\bbP(Z \geq z) \leq \phi(z)(\frac{1}{z}  - \frac{1}{z^3} + \frac{3}{z^5} ).$
\end{proof}
...
\begin{lem}[Polynomial bound and Chernoff bound]\label{lem:momcher}
	Let $X \geq 0$ be a non-negative variable. Suppose that the moment generating function of $X$, denoted $\varphi_{X}(\lambda)$, exists in the neighborhood of $\lambda = 0$. Given some $\delta > 0$, we have
...
\begin{proof}
	...
	Therefore, the moment  $\bbE[|X|^k]$ exists for all $ k \in \mathbb{Z_+}$. Applying power series~\eqref{eq:power} to the right hand side of equation~\eqref{eq:lempoly} yields
	...
	\end{equation}
	Therefore, the equation~\eqref{eq:lempoly} follows by combining the equation~\eqref{eq:powertopoly} with equation~\eqref{eq:cauchy3}.	
\end{proof}

\begin{rmk}
	Applying different functions $g(X)$ to the Markov's inequality leads to different bounds for the tail probability of variable $X$. Equation~\eqref{eq:lempoly} implies that optimized polynomial bound is tighter than the Chernoff bound, provided that the moment generating function of $X$ exsits.  
\end{rmk}

\subsection{Exercise 2.4}\label{pro:boundvar}
In Exercise~\ref{pro:boundvar}, we prove the Theorem~\ref{thm:boundvar},  the Hoeffding bound for a bounded variable. 

\begin{proof}
	
Suppose $X$ is a bounded random variable, where $X \in [a,b]$ almost surely, and $a 
...
\end{align}
We interpret the second-order derivative $g''(\lambda)$ interpreted as the variance of $X$ with the re-weighted distribution $d P' = {e^{\lambda X}}/{\bbE[ e^{\lambda X} ]} d P_X $, where $P_X$ is the distribution of $X$. Taking integral of 1 with respect to $dP'$, we have
\begin{equation}
	\int_{-\infty}^{+\infty} d P' = \int_{-\infty}^{+\infty}  \frac{e^{\lambda X}}{\bbE[ e^{\lambda X} ]} d P_X = 1,
\end{equation} 
which implies that the function $P'$ is indeed a distribution. Under all possible re-weighted distributions, the variance of $X$ is upper bounded as following,
\[ var(X) = var(X - \frac{a+b}{2}) \leq \bbE[(X - \frac{a+b}{2} )^2] \leq \frac{(b-a)^2}{4}, \]
where the term $\frac{(b-a)^2}{4}$ follows by letting $X$ distribute only on the boundary $a$ and $b$. Hence, the second-order derivative $g''(\lambda) \leq \frac{(b-a)^2}{4}$. We plug the results of $g'$ and $g''$ into the equation~\eqref{eq:taylor}. Then,
...
\end{equation}
where $g'' (\lambda)$ is the variance of $X$ with with the re-weighted distribution $d P' = {e^{\lambda X}}/{\bbE[ e^{\lambda X} ]} d P_X $, where $P_X$ is the distribution of $X$ and $g'' (0) = var(X)$. Therefore, $\max_{\lambda}g''(\lambda) = \sigma_{min}^2$. To let the equality $var(X) = \sigma_{min}^2$ hold, we need to show that $\max_{\lambda}g''(\lambda) = g''(0)$ holds for $X$. 
...
The variance of $Y$ with $dP'$ is ${2}/{3\bbE[ e^{Y} ]} \times  {e}/{3\bbE[ e^{Y} ]} = 0.2442 > 2/9$. Therefore, for this variable $Y$, we have $var(Y) <  g''(1) \leq \max_{\lambda}g''(\lambda) = \sigma^2_{min}$. 
\begin{rmk}
...
\begin{lem}[Lower bounds on squared sub-Gaussians]\label{lem:lowersquare} Let $\{ X_i \}_{i=1}^n$ be an i.i.d.\ sequence of zero-mean sub-Gaussian variables with parameter $\sigma$. The normalized summation $Z_n = \frac{1}{n} \sum_{i=1}^n X_i^2$ satisfies
\begin{equation}\label{eq:lowersquare}
	\bbP[Z_n - \bbE[Z_n] \leq \sigma^2 \delta ] \leq e^{-n\delta^2 /16}, \quad \text{ for all } \delta \geq 0.
\end{equation}
	The equation~\eqref{eq:lowersquare} implies that the lower tail of the summation of squared sub-Gaussian variables behave in a sub-Gaussian way.
\end{lem}

\begin{proof}
	Since $X_i^2$ are i.i.d.\ nonnegative variables, we apply the equation~\eqref{eq:thmonber0} in Theorem~\ref{thm:oneber} to the variables $X_i^2$, where $i \in [n]$. Then,
	\begin{equation}\label{eq:e26a}
		\bbP[\sum_{i=1}^n (X_i^2 -\bbE[X_i^2]) \leq n \sigma^2 \delta] \leq exp\left\{ - \frac{n\delta^2 \sigma^4}{ \bbE[X_1^4]} \right\},  \quad \text{for all }\delta \geq 0.
	\end{equation}
	By the equation~\eqref{eq:thmmomsubg} in Theorem~\ref{thm:momsubg}, we have
	\begin{equation}\label{eq:e26x4}
		\bbE[X_1^4] \leq 16 \sigma^4. 
	\end{equation}
	Combing equations~\eqref{eq:e26a}, \eqref{eq:e26x4}, and the definition of $Z_n$, we have
	\begin{equation}
...
\begin{rmk}
	Equation~\eqref{eq:lowersquare} implies that the lower tail of the summation of squared sub-Gaussian variables behave in a sub-Gaussian way. In following sections, we will show that the variable $Z_n - \bbE[Z_n]$ in Lemma~\ref{lem:lowersquare} is a sub-exponential variable.
\end{rmk}
